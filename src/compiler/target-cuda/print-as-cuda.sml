...
structure PrintAsCUDA : sig
    ...
    fun rawTyName rty = (case rty
           of RawTypes.RT_Int8 => "signed char"
            | RawTypes.RT_UInt8 => "unsigned char"
            | RawTypes.RT_Int16 => "short"
            | RawTypes.RT_UInt16 => "unsigned short"
            | RawTypes.RT_Int32 => "int"
            | RawTypes.RT_UInt32 => "unsigned int"
            | RawTypes.RT_Int64 => "long"
            | RawTypes.RT_UInt64 => "unsigned long"
  ...
  (* convert a type plus optional varaible to a string *)
    and tyToS (ty, optVar)= let
          ...
          and toS (ty, acc) = (case ty
                 of CL.T_Num nty => prefix(rawTyName nty, acc)
                  ...
                  | CL.T_Vec(ty, size) =>
                      prefix(concat([(tyToS' ty), (Int.toString size)]), acc)
  ...
  (* convert a type to a string *)
    ...
    fun output (_, CL.D_Verbatim[]) = ()
      | output (strm, decl) = let
          ...
          fun ppTyLast ty =  let
                val tyPrint = tyToS (ty, NONE)
                val s = size tyPrint
                in
                  str (substring(tyPrint, s-1,1))
                end
          fun ppAttrs [] = ()
            | ppAttrs attrs = (
        ...
        (* force printing "{" "}" around a statement *)
          ...
          and ppExp e = (case e
                 of CL.E_Grp e => (str "("; ppExp e; str ")")
                  ...
                  | CL.E_VecConstIndex(e, i) => (
                      ...
                      case i
                       of 0 => str ".x"
                        ...
                        | 2 => str ".z"
                        | 3 => str ".w"
                        | _ => raise Fail "Invalid vector index"
                      (* end case *))
                  ...
                  | CL.E_Vec(ty, args) => (
                    (* Cuda vector syntax: "make_floatN(a, b, ...)" *)
                      str "make_float"; (ppTyLast ty);
                      str "(";
                      PP.openHOVBox strm indent;
                        PP.cut strm;
                        ppCommaList {
