        ...
        (* generate the type and member declarations for a recorded type *)
          fun genDecl (ty, (tyDcls, fnDefs)) = (case ty
                 of Ty.VecTy(w, pw) => let
                      ...
                      val cudaTyName = concat[realTyName, Int.toString(w)]
                      val typedefDcl = CL.D_Verbatim[concat[
                              "typedef ", cudaTyName, " ", cTyName, ";"
                            ]]
                      fun mkWrapperParam name = CL.PARAM(["const"], CL.T_Ref cTy, name)
                      fun mkBinOpWrapper(symbol, binop) =
                        let
                          val v0 = CL.mkVar "v0"
                          val v1 = CL.mkVar "v1"
                          fun mkOp i = CL.mkBinOp(CL.mkVecIndex(v0, i), binop, CL.mkVecIndex(v1, i))
                          fun mkOps 0 = [mkOp 0]
                      ...
                      val negWrapperDcl =
                        let
                          val v = CL.mkVar "v"
                          fun mkOp i = CL.mkUnOp(CL.%-, CL.mkVecIndex(v, i))
                          fun mkOps 0 = [mkOp 0]
                            | mkOps i = (mkOps (i-1)) @ [mkOp i]
                          val body = mkReturn (CL.mkApply (concat["make_", cudaTyName], mkOps(w-1)))
                        in
                      ...
                      val elemMultWrapperDcl = mkBinOpWrapper("*", CL.#*)
                      in
                        (typedefDcl :: negWrapperDcl :: addWrapperDcl :: subWrapperDcl :: elemMultWrapperDcl :: tyDcls, fnDefs)
                      end
  ...
  (* builds AST for the expression "(x <= lo) ? lo : (hi <= x) ? hi : x;" *)
    ...
    fun doOp env (rator, dcls) = let
          ...
          fun mkVMap (ty, name, f, w, pw) = let
                fun f' i = CL.mkApply(f, [CL.mkVecIndex(CL.mkVar "v", i)])
                in
          ...
          val dcl = (case rator
                 of Print(Ty.TensorRefTy shape) => genTensorPrinter shape
                  ...
                  | VScale(w, pw) => let
                      ...
                      in
                        mkFunc(cTy, RN.vscale w,
                          ...
                          mkReturn(mkVec(w, pw, fn i => CL.mkBinOp(CL.mkVecIndex(CL.mkVar "v", i), CL.#*, CL.mkVar "s"))))
                      end
                  | VSum(w, pw) => let
                      val name = RN.vsum w
                  ...
                  | VDot(w, pw) => let
                      ...
                      fun mkSum 0 = CL.mkGrp(CL.mkBinOp(CL.mkVecIndex(CL.mkVar "u", 0), CL.#*, CL.mkVecIndex(CL.mkVar "v", 0)))
                        | mkSum i = CL.mkBinOp(mkSum(i-1), CL.#+, CL.mkGrp(CL.mkBinOp(CL.mkVecIndex(CL.mkVar "u", i), CL.#*, CL.mkVecIndex(CL.mkVar "v", i))))
                      in
                  ...
                  | VCeiling(w, pw) => mkVMap (RN.vecTy w, RN.vceiling w, "::ceil", w, pw)
                  | VFloor(w, pw) => mkVMap (RN.vecTy w, RN.vfloor w, "::floor", w, pw)
                  | VRound(w, pw) => mkVMap (RN.vecTy w, RN.vround w, "::round", w, pw)
                  | VTrunc(w, pw) => mkVMap (RN.vecTy w, RN.vtrunc w, "::trunc", w, pw)
                  ...
                  | VLoad(w, pw) => let
                      val name = RN.vload w
                      val cTy = RN.vecTy w
                      fun arg i = CL.mkSubscript(CL.mkVar "v", mkInt(i))
                      in
                        mkFunc(cTy, name,
                          [CL.PARAM(["const"], (CL.T_Ptr(realTy)), "v")],
                            ...
                            if (i < dim)
                              then if (w < pw)
                                then let
                                  val x = if (pw = 1)
                                        then CL.mkVar v
                                        else CL.mkVecIndex(CL.mkVar v, w)
                                  in
                      ...
                      in
                        CL.D_Template([CL.TypeParam "TY", CL.ConstParam(CL.intTy, "VOXSZ")],
                          mkFunc(CL.boolTy, RN.inside(dim, s),
                            vParams @ [CL.PARAM([], CL.T_Ref(CL.T_Const(imageTy (realTy, dim))), "img")],
                            mkReturn exp))
                      end
